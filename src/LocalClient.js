...
class LocalClient extends GenericClient {
  ...
  async _buildHeaders() {
    this.localHeaders = {
      Authorization:
        "Basic " +
        Buffer.from(`riot:${this._lockfile.password}`, "utf8").toString(
          "base64"
        ),
    ...
    let response = await axios.get(
      `${this._lockfile.protocol}://127.0.0.1:${this._lockfile.port}/entitlements/v1/token`,
      {
  ...
  async getPresences() {
    ...
    return presences.presences
      ...
      .map((presence) => {
        presence.private = JSON.parse(
          Buffer.from(presence.private, "base64").toString()
        );
  ...
  async getPresence(puuid) {
    ...
    return presences.find(
      (presence) => presence.puuid == (puuid ? puuid : this.puuid)
    );
  }
