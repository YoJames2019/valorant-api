...
class GenericClient {
  ...
  async activateContract(contractId) {
    return await this.fetch(
      `/contracts/v1/contracts/${this.puuid}/special/${contractId}`,
      "pd"
    );
  }
  ...
  async coregameDisassociatePlayer(matchId) {
    return await this.fetch(
      `/core-game/v1/players/${this.puuid}/disassociate/${matchId}`,
      "glz"
    );
  }
  ...
  async fetchMatchHistory(puuid, startIndex = 0, endIndex = 15, queueId) {
    ...
    return await this.fetch(
      `/match-history/v1/history/${puuid}?startIndex=${startIndex}&endIndex=${endIndex}` +
  ...
  async fetchCompetitiveUpdates(puuid, startIndex = 0, endIndex = 15, queueId) {
    ...
    return await this.fetch(
      `/mmr/v1/players/${puuid}/competitiveupdates?startIndex=${startIndex}&endIndex=${endIndex}` +
        (queueId ? `&queue=${queueId}` : ""),
      "pd"
    );
  }
  ...
  async fetchLeaderboard(seasonId, startIndex = 0, size = 1000) {
    return await this.fetch(
      `/mmr/v1/leaderboards/affinity/${this._region}/queue/competitive/season/${seasonId}?startIndex=${startIndex}&size=${size}`,
      "pd"
    );
  }
  ...
  async setPartyQueue(partyId, queueId = "unrated") {
    return await this.fetch(
      `/parties/v1/parties/${partyId}/makedefault?queueID=${queueId}`,
      "glz",
      "post"
    );
  }
  ...
  async setCustomGameSettings(
    ...
    return await this.fetch(
      `/parties/v1/parties/${partyId}/customgamesettings`,
      "glz",
      "post",
      {
  ...
  async leaveParty(partyId) {
    return await this.fetch(
      `/parties/v1/players/${this.puuid}/leaveparty/${partyId}`,
      "glz",
      "post"
    );
  }
  ...
  async inviteToPartyByDisplayName(partyId, username, tag) {
    return await this.fetch(
      `/parties/v1/parties/${partyId}/invites/name/${username}/tag/${tag}`,
      "glz",
      "post"
    );
  }
  ...
  async selectCharacter(matchId, characterId) {
    return await this.fetch(
      `/pregame/v1/matches/${matchId}/select/${characterId}`,
      "glz",
      "post"
    );
  }
  ...
  async lockCharacter(matchId, characterId) {
    return await this.fetch(
      `/pregame/v1/matches/${matchId}/lock/${characterId}`,
      "glz",
      "post"
    );
  }
  ...
  async fetchStoreEntitlements(
    ...
    return await this.fetch(
      `/store/v1/entitlements/${puuid}/${itemType}`,
      "pd"
    );
  }
